...
const PriorityBadge = ({ priority }) => {
  ...
  return (
    <span className="badge" style={styles[priority] || {}}>
      {priority}
    </span>
  );
};
...
export default function Maintenance() {
  ...
  const [isDetailsLoading, setIsDetailsLoading] = useState(false); // 👈 loader del modal
  ...
  // 👇 Detalles con datos frescos + loader
  const handleOpenDetails = async (request) => {
    setIsDetailsLoading(true);
    setSelectedRequest(null);
    try {
      const detailedRequest = await retrieveMaintenanceRequest(request.id);
  ...
  async function onSubmit(e) {
    e.preventDefault();
    try {
      const newRequest = await createMaintenanceRequest(form);
      toast.success("Solicitud enviada, subiendo adjuntos...");

      if (files.length > 0) {
        const uploadPromises = files.map((file) =>
          uploadMaintenanceAttachment(newRequest.id, file)
        );
  ...
  return (
    <div style={{ padding: 24, display: "grid", gap: 24 }}>
      ...
      {/* Reporte para usuarios no admin */}
      {!isAdmin && (
          ...
          >
            ...
            <div>
              ...
              <button
                ...
                style={{ marginTop: "4px", background: "var(--silver-500)" }}
              >
          ...
          <tbody>
            {requests.map((req) => (
              <tr key={req.id}>
                ...
                <td>
                  <PriorityBadge priority={req.priority_display} />
                </td>
                <td>
                ...
                <td>
                  ...
                  >
                    <button onClick={() => handleOpenDetails(req)}>
                      Ver detalles
                    </button>
                    ...
                    {isAdmin && (
                      <>
                        <select
                          value={req.priority}
                          onChange={(e) =>
                            handlePriorityChange(req.id, e.target.value)
                          }
                        >
                          <option value="BAJA">Baja</option>
                          <option value="MEDIA">Media</option>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            handleAssignWorker(req.id, e.target.value)
                          }
                        >
            ...
            {!requests.length && (
              <tr>
                {/* 👇 corrige colSpan cuando no hay filas */}
                <td colSpan={isAdmin ? 6 : 5}>No hay solicitudes.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {/* Modal / tarjeta de detalles con loader */}
      <div
        className="card"
        style={{
          maxWidth: 600,
          justifySelf: "center",
          marginTop: "16px",
          display: isDetailsLoading || selectedRequest ? "block" : "none",
        }}
      >
        {isDetailsLoading ? (
          ...
          selectedRequest && (
            <>
              ...
              <p>
                <b>Descripción:</b> {selectedRequest.description}
              </p>
              <p>
                <b>Reportado por:</b> {selectedRequest.reported_by_username}
              </p>
              <p>
                <b>Asignado a:</b>{" "}
                {selectedRequest.assigned_to_username || "Nadie"}
              </p>
              <p>
                <b>Prioridad:</b>{" "}
                <PriorityBadge priority={selectedRequest.priority_display} />
              </p>
              <p>
                <b>Estado:</b>{" "}
              ...
              {selectedRequest.status === "COMPLETED" && (
                <p>
                  <b>Completado por:</b>{" "}
                  {selectedRequest.completed_by_username} el{" "}
                  {new Date(
              ...
              >
                <textarea
                  ...
                  onChange={(e) =>
                    setCommentForm({ body: e.target.value })
                  }
                  style={{ flex: 1 }}
                />
